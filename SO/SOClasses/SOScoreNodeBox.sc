SOScoreNodeBox {
	...
	*new { arg so, path, pdf = true, pdfPath,
				...
				width = 800, height = 400, title, update = true ;			^super.new.initSONodeBox(so, path,	pdf, pdfPath,		
	...
	initSONodeBox { arg aSo, aPath, aPdf, aPdfPath, 
		...
		path 	= if (aPath.isNil, { so.soundPath.split($.)[0]++".py"}, { aPath }) ;
		pdf = aPdf ;
...
//________________________________________________________________
	...
	drawTickLabels { arg timeInterval, from, to, timeRatio ; 
		var number = (duration/timeInterval).asInteger ;
		var label, pt = 10 ;
		file.write( format("font(\"%\", %)\n", "Helvetica", pt) );
       	file.write("fill(0, 0, 0)\n");
		number.do({ arg n ;	
			label = (timeInterval*(n+1)+from).asTimeString ;
			file.write( format("text(\"%\", %, %)\n",label,
				timeInterval*timeRatio* (n+1)-(from*timeRatio)+3,
				 height-pt-3)
				) ;
		})			
	} 
...
//________________________________________________________________
	drawProfile { arg objectDef, from, to, timeRatio, x, h, w, y, hStep ; 
		...
		file.write(format("beginpath(%, %)\n", x, y)) ;
		file.write(format("lineto(%, %)\n", x+w, y+chunk)) ;
		file.write(format("lineto(%, %)\n", x+w, y+h)) ;
		file.write(format("lineto(%, %)\n", x, y+h)) ;
			file.write(format("lineto(%, %)\n", x, y)) ;
		file.write("endpath()\n") ;
