...
//		processDict.removeAt(\sound) ;
	...
	begin { arg processName ;
		...
		{	
			{ this.gui.cleanGui }.defer ;
			// note that cleanGui must be performed in 0.25
			// fade out 0.25 sec
			10.do{|i| Server.local.volume.volume_((i*8).neg) ; 0.025.wait } ;
			// clear eventual stuff, e.g. busses  
			processDict[processName].end ;
			TempoClock.clear ; // processes are scheduled on TempoClock
			TempoClock.default.tempo = 1 ; // in case some process modifies it and it gets stuck 
			AppClock.clear ; // monitor tasks...
			// SystemClock.clear ; 
			// we leave SystemClock, usefulto force stuff in end msg
			Server.local.freeAll ;
			Server.local.sync ;
			Buffer.freeAll ;
			Server.local.sync ;
			// back to default vol
			Server.local.volume.volume_(0) ; 
			mixer.recreateSynths ;	// back to clean mixer 
		}.fork
	} 
...
SoundProcessGui2 {
	...
	*new { arg step = 20, rate = 5 ; 
	...
	cleanGui { window.view.removeAll }
