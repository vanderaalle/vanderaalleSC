	...
	*new { arg path, logPath, thresh = 0.5 ;
	...
	initOA { arg aPath, aLogPath, aThresh ;
		...
		SynthDef(\listener, { arg in, thresh = 0.5;
				...
				SendTrig.kr(Onsets.kr(chain, thresh), 999, Lag.kr(Amplitude.kr(sig)).ampdb);
			}).add ;
		...
		resp = OSCFunc({ arg msg, time;
				onsets = onsets.add([(time-startTime).postln, msg.postln[3]]) ;
	...
	thresh_{|v| listener.set(\thresh, thresh) ; thresh = v}
