/*
THE BABAKOTO PROJECT: BabaPatcherGui
-a- started: ~20/06/08
...
// for placing algo
	var <>lines, <>linesPerCol ; // the total lines (by counting nexline) 
	var col, totalLines ;
	var <>startConnection ;

	
	*new { arg patcher, widthInModules = 3, height = 1000, vStep = 15 ;
		^super.new.initBabaPatcherGui(patcher, widthInModules, height, vStep) 
	}

	initBabaPatcherGui { arg aPatcher, aWidthInModules, aHeight, aVStep ;
		widthInModules = aWidthInModules ;
		height = aHeight ;
	 	vStep = aVStep ;
	 	linesPerCol = height/vStep ;
		patcher = aPatcher ;
		// should be moved to makeAllGui/MakeGuiFromList
		window = GUI.window.new(patcher.name.asString, 
			Rect(30,30, (400+vStep)*widthInModules, height)
			//, scroll:true
			) ;
		flow = FlowLayout.new( window.view.bounds );
	    	window.view.decorator = flow;
	     window.view.background_( Color(0, 0, 0.3)) ;
	     // end of 
	...
	makeAllGui {
		...
		patcher.synthDict.keys.do ({ |name| 
