...
RuLooper {
	...
	*new { arg dataSender, dur, repeat = inf, quant = 0.1, timeScale = 1  ; 
	...
	makeGui { arg rate = 0.02 ;
		gui = RuLooperGUI(this).fillWindow.drawRate_(rate).drawHistory ;
...
RuLooperGUI {
	...
	*new { arg looper, wStep = 15, hStep = 25, offset = 40 ;  
	...
	fillWindow {
		...
		window.bounds_(Rect(200, 200, numPix*wStep, 18*hStep)) ;
		18.do({ |i|
		...
		barLine = UserView.new(window, Rect(offset, 0, wStep, hStep*18))
	...
	update { arg theChanged, theChanger, more ;
		...
		*/
			{ [\replaceHistory, \mergeHistory, \eraseHistory].postln.includes(more[0]).postln}
				{
				this.drawHistory ; 
				old = nil ;
				oldVal = 1 ; // protection
			...
			{ more[0] == \sync }
				...
				barLine.bounds_(Rect(offset, 0, wStep, hStep*18)) 
				}
			{ more[0] == \play }
				{
				playTask.play ;
