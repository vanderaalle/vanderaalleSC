...
VepRunnerStereo {
	...
	*new { arg folder, type = "wav", numSources, vol = 1, subVol = 0.25,
			...
			*/ 
			speakerPositions = [pi/6, pi.neg/6],
			spread = 0,
...
// we asssume that dist is passed in a normalized range [0,1]
			...
			SynthDef(\vepPlayer, { arg in = 1, out, vol = 1, dist = 0, 
				...
				numChans = 2, azimuth = 0, elevation = 0, spread = 0, vbapBuf ;
				...
				*/ 
				...
				Out.ar(out, VBAP.ar(2, distancedSource, vbapBuf, azimuth, elevation, spread)					);
			}) ;
