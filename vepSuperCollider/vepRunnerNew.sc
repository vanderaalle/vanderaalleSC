...
VepRunnerNEW {
	...
	var <>folder ; // all the files in the folder with type extension will be loaded
	var <>type ; // audio file extension
	...
	var <>delArray ; // for delays
	
	*new { arg folder, type = "wav", numSources, vol = 1, subVol = 0.25,
			def, 
			...
			*/ 
			...
			maxDist = 30 ;
		^super.new.initVepRunner(folder, type, numSources, def, dim, speakerPositions, spread, maxDist, true) 
	}


	// same but do not set load stuff. Useful for register a depenendancy before syncing
	*newPaused { arg folder, type = "wav", numSources, vol = 1, subVol = 0.25,
			def, 
			dim  = 2,  
			speakerPositions = [0, pi/4, pi/2, (3/4)*pi, pi, (3/4)*pi.neg, pi.neg/2, pi.neg/4],
			spread = 0,
			maxDist = 30 ;
		^super.new.initVepRunner(folder, type, numSources, def, dim, speakerPositions, spread, maxDist, false) 
	}

	

	initVepRunner { arg aFolder, aType, aNumSources, aDef,  aDim, aSpeakerPositions, aSpread, aMaxDist, sync  ; 
		folder = aFolder ;
		type = aType ;
		if (folder.isNil) {"\n\nWARNING! Folder is empty!".postln} ;
...
// we asssume that dist is passed in a normalized range [0,1]
			...
			SynthDef(\vepPlayer, { arg in = 1, out, vol = 1, dist = 0, 
				...
				del = 0, maxDel = 0 ; // delay for source distance 
				...
				*/ 
				...
				Out.ar(out, DelayL.ar(VBAP.ar(8, distancedSource, vbapBuf, azimuth, elevation, spread), maxDel, del)					);
			}) ;
		...
		delArray = Array.fill(numSources, {0}) ; // delay times are set to 0
	...
	syncToServer {
		...
		def.send(Server.local) ; // send the def
		Server.local.sync ;  
		this.loadTracks(type) ; // load the tacks into buffers
		"Tracks: \t\tOK".postln ; 
		Server.local.sync ;
	...
	loadTracks {  arg type ;
		var labelList = SampleListGenerator.fromPath(folder) ;
		"\n\nLoading this tracks: ".postln ;
		labelList.do{|l| "\t".post;l.postln } ;
		// Here we set the buffers, one for each track
		trackArr = Array.fill(labelList.size, Ê
			{ |i| Buffer.read(Server.local, folder++labelList[i]++"."++type) }) ;
	}
	...
	addSource { arg defName ;
		...
		sources = sources.add( 
			VepSource.new(defName)
				.set(\vbapBuf, vbapBuffer.bufnum)
				.set(\out, outBusArr[0])
				.set(\maxDel, maxDist/340) // max delay depends on distance
		) ;
...
// PLAYERS
	...
	// so we can route them to vepSources
		...
		playerBusArr = Array.fill(4, { Bus.audio }) ;
		
		Server.local.sync ;
		
		// routing
		playerArr = Array.fill(4, {|i| 
...
// SOURCES CONTROL
	...
	stopSource { arg sourceIndex ;
		...
		var del = delArray[sourceIndex].postln  ; 
		actives.removeAt(sourceIndex)  ; // out of actives
		source.state_(\off) ; // attribute
		source.set(\release, 0.5+del) ; // 0.5 is default 
		source.set(\trig, 0) ;
			// WE DO NOT WE NEED THIS
			// BECAUSE DEF HAS CHANGED: doneAction:1 ACHTUNG
			// source.run(false) ;
	...
	updateSource { arg sourceIndex, x, y, z ;
		var theta, rho, source, del ;
		#theta, rho = this.thetaRho(x,y,z) ;
		del = rho/340 ;
		source = sources[sourceIndex] ;
		delArray[sourceIndex] = del ;
		source.synth.set(
			\azimuth, theta.raddegree, 
			\dist, rho.linlin(0, maxDist, 0, 1), 
			\maxDel, del,
			\del, del*2) ; 
