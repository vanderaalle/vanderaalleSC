...
SapphireMonitor {
	...
	*new { arg runner, sampleTime = 0.5 ; ^super.new.initSapphireMonitor(runner, sampleTime) }
	
	initSapphireMonitor { arg aRunner, aSampleTime ;
		var usr ;
		runner = aRunner ;
		...
		{ GUI.id === \swing } {
			...
			stetho = Stethoscope.new(Server.local,4, view: bck) ;
		...
		{ GUI.id === \qt } {
			...
			stetho = 	QStethoscope2bis.new(Server.local,4, view: bck, 
