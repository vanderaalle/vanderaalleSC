// dirty class for the fegato project
// focuses on pitch class notation, febr 2018
...
".replace("HEIGHT", data.size*0.75).replace("WIDTH", (amp.size/4)*0.75) ;
	...
	createSequence {|voice|
		...
		var actual, chordStr = "" ;
		var durs = ["16","8","8.","4"] ;
		var v = voice.collect{|i| i.sort}.add([\fake]) ;
		//fake allows skipping later
		var chName = "", struct ;
		// -2 but the last is \fake
		v[..v.size-2].do{|i, j|
			if(v[j+1] == i ) {
				if(j%4 == 3) {
					"siamo in sonaLilyclass".postln ;
					if(i.postln.size>0){
						struct = ChordAnalyzer.analyze(i) ;
						chName = if (struct.notNil){
				ChordAnalyzer.convertName(ChordAnalyzer.analyze(i))
						}{""}
					} ;
					chordStr = chordStr+this.createLilyChord(i)
					++durs[cnt] ;
					if(chName.size >0){chordStr = chordStr++"^\""++chName++"\""};
					// not a pause
