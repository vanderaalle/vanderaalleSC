...
IM4Performer {
	...
	var <>doors ; // serial port
	var <>scanTime = 0.05 ; // scan time
	...
	*new { arg aPath ; // where the folder with score files is placed
	...
	initIM4Performer { arg path ;
		doors = [
			SerialPort("/dev/tty.usbmodem24111", 9600),
			SerialPort("/dev/tty.usbmodem24121", 9600),
			SerialPort("/dev/tty.usbmodem24131", 9600)
		] ;
	...
	play { arg array ;
		...
		task = Task{
			var id, idx, voice, counter, amp, freq ;
			var radio = 4, msg = 2, doorID ;
			var door ;
			inf.do{|i|
				counter = i * scanTime ;
				//doorID = ((i % (msg * radio * doors.size)) / doors.size).asInteger ;
				doorID = ((i / 4).asInteger) % doors.size ;
				"Arduino: ".post;
				door = doors[doorID.postln] ;
				idx = i % array.size ;
				id = (array[idx] - 1) % 4 ;
				"Voice: ".post ; id.postln ;
				voice = voiceArr[id] ;
				amp = voice.finAmp.at(counter) ;
				freq = voice.finFreq.at(counter) ;
				door.put(id) ;
				0.5 * scanTime.wait ;
				// was 106
				door.put((((255 - (amp * 17)).log10 * 106).round*0.1)) ;
				amp.postln ;
				0.5 * scanTime.wait ;
				/*door.put(freq) ;
				freq.postln ;
